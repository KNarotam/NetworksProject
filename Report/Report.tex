...
%These commands served as the fundamental concepts on which the server framework was implemented. The designed FTP server was hosted and run locally using the users local IP address as well as using \textit{FileZilla}, an open source application with FTP capabilities. The server also implemented multi-threading in order to facilitate multiple client connections simultaneously. This allowed for the concurrent upload and download of files through multiple data connections. 
...
\begin{longtable}{|p{2cm}|p{7cm}|p{8cm}|}
	\hline 
	\textbf{Command} & \textbf{Description} & \textbf{Reply Code}
	\hline 		
	USER & User identification needed by server before providing access to file system  &  501 Syntax error in parameters or arguments. 331 User name okay, need password. \tabularnewline
	\hline 
	PASS & Authenticates user password & \makecell[l]{501 Syntax error in parameters or arguments.\\503 Bad sequence of commands. \tabularnewline 230 User logged in, proceed.} \tabularnewline 
	\hline 
